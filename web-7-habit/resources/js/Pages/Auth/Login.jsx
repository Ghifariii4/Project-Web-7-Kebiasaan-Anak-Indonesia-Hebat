...
export default function Login() {
    ...
    const submit = (e) => {
        e.preventDefault();
        console.log("Login Data:", { role, guruType, ...data });
        // post(route('login')); 
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative overflow-hidden transition-colors duration-700">
            ...
            <div className={`fixed top-0 left-0 w-96 h-96 rounded-full blur-3xl opacity-30 -translate-x-1/2 -translate-y-1/2 transition-colors duration-1000 ${activeTheme.blob}`}></div>
            <div className={`fixed bottom-0 right-0 w-96 h-96 rounded-full blur-3xl opacity-30 translate-x-1/3 translate-y-1/3 transition-colors duration-1000 ${activeTheme.blob}`}></div>
            ...
            <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-xl border border-white relative z-10 overflow-hidden transition-all duration-500">
                ...
                {/* --- HEADER DINAMIS --- */}
                <div className={`${activeTheme.bg} p-8 pb-12 text-center text-white relative transition-colors duration-500 ease-in-out`}>
                    ...
                    <p className={`${activeTheme.textLight} text-sm mt-1 transition-colors duration-500`}>Silakan masuk sesuai peran Anda.</p>
                </div>
                ...
                <div className="p-8 -mt-8 bg-white rounded-t-[2.5rem] relative">
                    ...
                    <div className="grid grid-cols-3 gap-2 p-1.5 bg-slate-50 rounded-2xl mb-8 border border-slate-100">
                        {['siswa', 'guru', 'orangtua'].map((item) => (
                            ...
                            >
                                <div className={`transition-colors duration-300 ${role === item ? roleThemes[item].text : "text-slate-400 group-hover:text-slate-500"}`}>
                                    {item === 'siswa' && <GraduationCap size={20} className="mb-1" />}
                                    {item === 'guru' && <Presentation size={20} className="mb-1" />}
                                    {item === 'orangtua' && <UserCheck size={20} className="mb-1" />}
                                </div>
                                <span className={`text-[10px] font-bold uppercase tracking-wider transition-colors duration-300 ${role === item ? roleThemes[item].text : ""}`}>
                    ...
                    <form onSubmit={submit} className="space-y-5">
                        ...
                        {role === "siswa" && (
                            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                                <Label className="text-slate-600 font-semibold ml-1">NIS (Nomor Induk Siswa)</Label>
                                <Input 
                                    type="text" 
                                    inputMode="numeric" 
                                    pattern="[0-9]*"
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <Label className="text-slate-600 font-semibold ml-1">
                                            {guruType === 'pns' ? 'NIP (Nomor Induk Pegawai)' : 'Nama Lengkap'}
                                        </Label>
                                        ...
                                        <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200">
                                            <button 
                                                type="button"
                                                onClick={() => { setGuruType('pns'); setData("identifier", ""); }}
                                            ...
                                            <button 
                                                type="button"
                                                onClick={() => { setGuruType('honorer'); setData("identifier", ""); }}
                                                className={`px-3 py-1 rounded-md text-[10px] font-bold transition-all duration-300 ${
                                                    guruType === 'honorer' 
                                                    ? 'bg-white text-blue-600 shadow-sm' 
                                                    : 'text-slate-400 hover:text-slate-600'
                                    ...
                                    <Input 
                                        key={guruType}
                                        type="text"
                                        inputMode={guruType === 'pns' ? "numeric" : "text"}
                                        placeholder={guruType === 'pns' ? 'Contoh: 19850101...' : 'Masukkan Nama Lengkap'} 
                        ...
                        {role === "orangtua" && (
                            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                                <Label className="text-slate-600 font-semibold ml-1">Email Terdaftar</Label>
                                <Input 
                                    type="email" 
                        ...
                        <div className="relative">
                            <Label className="text-slate-600 font-semibold ml-1">Kata Sandi</Label>
                            <div className="relative">
                                <Input 
                        ...
                        <Button 
                            ...
                            disabled={processing}
                        >
                            Masuk Sekarang <ArrowRight className="ml-2 w-5 h-5" />
                        </Button>
                        ...
                        <div className="text-center pt-2 h-6"> 
                            {role === "orangtua" ? (
                                <p className="text-slate-500 text-sm animate-in fade-in duration-300">
                                    Belum punya akun?{" "}
                                    <Link href="/register" className={`font-bold transition-colors duration-300 hover:underline ${activeTheme.text}`}>
                                        Daftar Orang Tua
                                    </Link>
                                </p>
                            ) : (
                                <p className="text-slate-400 text-xs animate-in fade-in duration-300">
                                    Lupa NIS/NIP/Password? Hubungi Admin Sekolah.
                                </p>
                            )}
                        </div>
